...
| completeness bijections [M3e] | arrange2d | 🚧 (`separating_boundary_edge_count` — edge-count **coverage**, not a full source-ID permutation; the component↔face / V_∂↔shell-vertex bijections + per-edge identity/orientation are deferred, see `docs/engineering-log.md`; `ledge_dom_certified` gate) | ✅ (segments/mixed/degree-6) | ✅ (CGAL faces+holes **+ exact a+b√d boundary vertices**, non-pinching) | — | — | — |
