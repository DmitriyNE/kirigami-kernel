...
| CLOSURE-CAP disjunction + CLOSURE_VALID(j) − SEW [M4] | closure | ✅ (`closure::valid`: `closure_cap` = MITER ∨ LEDGE (miter first, ledge fallback, both faults on `NoBranch`); `closure_valid` ANDs regularity ∧ CLIP-DOM(A,B) ∧ TRIM-LOCAL(A,B) ∧ CLOSURE-CAP, short-circuiting to the first fault; the 90° cylinder fold certifies through **both** cap branches, a flat joint refused at REG-V — 5 tests + doctest; generality corpus folds cylinder + 2 cone angles) | ⬜ | — | N/A (orchestration; soundness = the reused ★ branch checkers CAP-IN-D24 / regularity / MITER / CLIP-σ) | N/A | — |
