...
**Naming.** The project is **Kirigami** (the *Kirigami kernel*) — kirigami = cut *and* fold, matching a flat artwork that is cut, then folded/rolled (origami would only fold). Crates keep short, unprefixed names (`lattice`, `geom`, …): they are internal to the workspace and `publish = false`, so a `kirigami-` prefix buys nothing — adopt one only if a crate is ever published. The frozen normative spec keeps its own name, *Flex Substrate Representation, spec v0.24*, which Kirigami implements; the spec is not renamed (only a one-line implementation attribution was added to its informative preamble).

**`no_std` handling (approved).** `certify-core` and `lattice` are hard `#![no_std]` (invariant 4) with explicit `extern crate alloc`. The bignum backend does **not** get to relax this: `lattice` exposes its integer/rational as **newtypes over a backend trait**, so the `no_std + alloc` requirement lives at the `lattice` API and the backend (`dashu`, chosen at M0) is swappable behind it. **`no_std + alloc` compatibility is therefore a hard gate in the M0 backend benchmark** — added to the yardstick, which as written measured only Sturm speed on degree-12 over 256-bit rationals.
