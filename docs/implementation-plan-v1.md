...
- **Workspace layout:** `lattice`, `geom`, `certify1d`, `arrange2d`, `closure`, `sew`, `gate`, `develop`, `export`, `fixtures`, `difftest`. The spec lineage lives in-repo under `spec/`; the := census and the pattern-ledger lint list run in CI against both spec and code comments.
