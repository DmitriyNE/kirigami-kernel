...
| CAP-OUT-LINK / V_∂ membership (§8.5) | `arrange::{classify_link,link_ok,v_boundary}` | Kani (bounded) | ✅ `link_ok_iff_no_pinch` — over all sector masks ≤6, the O(n) run-counter agrees with the exhaustive O(n²) pinch definition (accept ⇒ selected sectors are one cyclic interval, no non-manifold pinch) |
| Link_emitted ≅ Link_geometric (§8.5) | `arrange::link_iso_ok` | Kani (bounded) | ✅ `link_iso_matches_cyclic_adjacency` — over all permutations N=4, the rotation-search matches the cyclic-adjacency reference (accept ⇒ identity-fixing oriented cyclic iso, not a mere multiset match) |
