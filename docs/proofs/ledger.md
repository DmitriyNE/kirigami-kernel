...
| CAP-OUT-LINK / V_∂ membership (§8.5) | `arrange::{classify_link,link_ok,v_boundary}` | Kani (bounded) + Aeneas lift | ✅ `link_ok_iff_no_pinch` — over all sector masks ≤6, the O(n) run-counter agrees with the exhaustive O(n²) pinch definition. Also Aeneas-lifted to `CertifyCore.Funs`; `CertifyCheck.CapOut` proves dispatch soundness axiom-clean (`link_ok` false ⇔ `Pinch`, `v_boundary` ⇔ `Boundary`, V_∂ ⊆ accepted). Run-counter refinement + 2-manifold thm = frontier |
