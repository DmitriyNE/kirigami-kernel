...
| [Fast-path gcd](fast-path.md#gcd) | `small::gcd_u128` | `GcdReduce.lean` | ⚠️ **OPEN — re-proof IN PROGRESS, contains `sorry` (OPT.3, 2026-08-15).** The Rust is now strip-twos + `u64`-narrowed Euclidean. **Done:** the `trailing_zeros` hole is filled by a faithful `def` in `Lattice/FunsExternal.lean` (+ `two_pow_natTrailingZeros_dvd` for its `u32` fit) — `check-externals.sh` green, 13 items modelled, still no `axiom`; and `gcd_u128_loop0_loop0_spec` (the inner `u64` Euclid) is **complete**, the original loop argument at the narrower width. **Not done:** `gcd_u128_loop0_spec` (outer loop) carries one `sorry` — a purely arithmetic side-goal, `gcd·2^shift < U128.size`, which its sibling branch closes with `scalar_tac` but which needs the operands re-associated after the cast rewrites; and `gcd_u128_spec` (the wrapper) is **unwritten** — it needs `gcd(2^i·m, 2^j·n) = 2^min(i,j)·gcd(m,n)` plus the link from `natTrailingZeros` to the odd decomposition. **Until this row is ✅ the gcd path is covered only by the Rust differential test (~80k pairs) and the bit-identical ε pins, not by Lean.** |
