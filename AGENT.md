...
3. **The extraction spike** (vv-guide §7) ⟵ **CURRENT**: sign-variation counter lifted both ways to Lean, proven; Sturm hypothesis-checker proven against a Mathlib citation; **plus** lift `verify_chain`/`verify_common_factor`/gcd-reduce correctness (the §8 lattice `*_correct_i16` obligations Lean now owns). Produce the go/no-go + the per-checker template.
