...
2. **M0 lattice** ⟵ **CURRENT** — *slice 1 done*: backend chosen by benchmark (**dashu**; passes the hard `no_std`+`alloc` gate, ties malachite for fastest — `docs/lattice-backend-benchmark.md`) behind `lattice::backend::Backend`; the two-tier `Int`/`Rat` L0 fast path + promotion and exact `cmp`/`sign`/`gcd` are implemented, differentially checked (fast≡slow + a 2nd backend) and Kani-verified panic-free. *Slice 2 (current)*: ℚ[x] polynomial arithmetic, **Sturm sequences** (isolation + sign-on-interval), **bivariate resultants**, interval-plus-separation comparison — plus the spike's loop-bounded-gcd hardening of the fast≡slow Kani harnesses. Acceptance criteria: `vv-guide §8`.
