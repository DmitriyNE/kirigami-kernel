...
mod ffi {
    unsafe extern "C++" {
        ...
        /// Build smoke: exercises a heavy CGAL header + its exact number type and
        /// gmp linking. Returns an exact rational as a string ("3/4").
        fn cgal_smoke() -> String;

        /// Circular-kernel `Arrangement_2` smoke: the intersection vertex of two
        /// crossing segments as two `a b d` triples (`a + b·√d`), proving the
        /// circular kernel builds/links and `Root_of_2` extraction works.
        fn cgal_arr_smoke() -> String;

        /// Differential oracle: the genuine (degree ≥ 3) intersection vertices of an
        /// arrangement of the input curves (one per line: `S x1 y1 x2 y2` segment /
        /// `C cx cy r2` circle, rationals as "num/den"), each returned as
        /// `xa xb xd ya yb yd` (coordinate = a + b·√d).
        fn cgal_arrange(input: &str) -> String;

        /// Overlap-edge oracle: one line per arrangement edge,
        ...
        /// Region/boolean oracle: the connected-component count of a boolean `op`
        /// (xor|and|or) over two disk operands (`C cx cy r2 operand`). See
        /// cgal_shim.cc for the pinch-semantics note.
        fn cgal_boolean_count(input: &str, op: &str) -> String;

        /// Structural cross-check: the total number of holes across all components
        /// of the boolean `op` (a `General_polygon_with_holes_2` counts its inner
        /// boundaries) — an annulus △ has exactly one. Same input as above.
