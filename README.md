...
Certified-exact geometry kernel in Rust; no floats in certified paths; every claim ships a checkable certificate. Soundness = verified *checkers* (pure `certify-core`, proven in Lean 4 via Rust→Lean lifting + Kani on the lattice) + tested *searchers* (`kernel-search`, checked at runtime). One honest caveat: the arrangement checkers verify the *combinatorial* certificate, not the geometric front-half that produces the coordinates (carrier/decompose/membership/coincidence) — that is differentially + property validated, not yet checker-certified (see `AGENT.md` §soundness and `docs/engineering-log.md`). The math is frozen at spec v0.24 after 24 adversarial review rounds — implement it, don't redesign it. Build order: lattice → arrangement kernel → charts → closure → sewing → gate. First task: repo skeleton + M0 lattice + the Lean-extraction spike (vv-guide §7).
